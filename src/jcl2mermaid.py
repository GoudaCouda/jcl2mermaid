...
def escape_d2_text(text: str) -> str:
    return (
        text.replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
...
def design_d2(job: JclJob) -> str:
    ...
    for step_idx, step in enumerate(job.steps):
        ...
        for dd in step.dds:
            for ds in dd.datasets:
                ...
                if filetype == "OUTPUT":
                    disp_str = f"\\nDISP: {edge_label_safe}" if edge_label_safe else ""
                    clean_dsn_esc = clean_dsn.replace('"', "'")
                    dd_name_esc = dd.name.replace('"', "'")
                    handoff_nodes[ds_id] = f"{dd_name_esc}: {clean_dsn_esc}{disp_str}"
                    data_edges.append(f"  {step_id} -> {ds_id}")
                elif ds_id in handoff_nodes:
                    data_edges.append(f"  {ds_id} -> {step_id}")
                elif filetype == "DELETE":
                    prefix = f"{dd.name}: " if dd.name else ""
                    deletes.append(f"{prefix}{clean_dsn}")
                elif filetype == "SCRATCH":
                    pass
                else:
                    prefix = f"{dd.name}: " if dd.name else ""
                    reads.append(f"{prefix}{clean_dsn}")

        # Construct Step Container
        step_name_esc = step.name.replace('"', "'")
        d2_lines.append(f'{step_id}: "{step_name_esc}" {{')
        d2_lines.append('  class: stepCard')
        
        # Program Badge
        step_pgm_esc = step.program.replace('"', "'")
        d2_lines.append(f'  pgm: "PGM: {step_pgm_esc}" {{')
        d2_lines.append('    class: pgmBadge')
        d2_lines.append('  }')

        if step.steplib:
            lib_str = ", ".join(step.steplib).replace('"', "'")
            d2_lines.append(f'  steplib: "LIB: {lib_str}" {{')
            d2_lines.append('    style.fill: transparent')
            d2_lines.append('    style.stroke-width: 0')
            d2_lines.append('    style.font-size: 10')
            d2_lines.append('    style.font-color: "#64748b"')
            d2_lines.append('  }')

        # In-stream DD Cards
        for dd in step.dds:
            if getattr(dd, "cards", None) and any(c.strip() for c in dd.cards):
                card_text = format_cards(dd.cards).replace('"', "'").replace('\\', '\\\\')
                dd_card_id = f"dd_{sanitize_id(dd.name)}"
                d2_lines.append(f'  {dd_card_id}: "{dd.name.upper()}:\\n{card_text}" {{')
                d2_lines.append('    class: codeBlock')
                d2_lines.append('  }')

        if reads:
            reads_text = "\\n".join(reads).replace('"', "'")
            d2_lines.append(f'  inputs: "External Input:\\n{reads_text}" {{')
            d2_lines.append('    class: inputBox')
            d2_lines.append('  }')

        if deletes:
            deletes_text = "\\n".join(deletes).replace('"', "'")
            d2_lines.append(f'  deletes: "Deletes:\\n{deletes_text}" {{')
            d2_lines.append('    class: deleteBox')
            d2_lines.append('  }')

        if step.comments:
            clean_comments = [
                f"• {c.lstrip('/*=- ').strip().replace('\"', '\'')}"
                for c in step.comments
                if c.strip()
            ]
            if clean_comments:
                comments_text = "\\n".join(clean_comments)
                d2_lines.append(f'  comments: "{comments_text}" {{')
                d2_lines.append('    class: commentBox')
                d2_lines.append('  }')
    ...
    # Handoff nodes
    if handoff_nodes:
        for ds_id, label in handoff_nodes.items():
            d2_lines.append(f'{ds_id}: "{label}" {{')
            d2_lines.append('  class: handoffCard')
            d2_lines.append('}')
        d2_lines.append('')
    ...
    if len(step_ids) > 1:
        for idx in range(len(step_ids) - 1):
            d2_lines.append(f"{step_ids[idx]} -> {step_ids[idx+1]}: {{")
            d2_lines.append('  style.stroke-dash: 4')
            d2_lines.append('}')
        d2_lines.append('')

    # Data flow connections
    if data_edges:
        d2_lines.extend(data_edges)
        d2_lines.append('')
...
def process_diagram(lines: Sequence[str], args) -> str:
    ...
    if designtype in ["graphviz", "dot"]:
        ...
        output_diagram = design_d2(jclJob)
    else:
